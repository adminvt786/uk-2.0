const { extractYouTubeID } = require('./string');
 ...
 */
const sanitizeUser = (entity, config = {}) => {
 ...
 * E.g. you should sanitize and encode URI if you are creating links from public data.
 */
const sanitizeListing = (entity, config = {}) => {
  const { attributes, ...restEntity } = entity;
 ...
 */
const sanitizeEntity = (entity, config) => {
  const { type } = entity;
  switch (type) {
    case 'listing':
